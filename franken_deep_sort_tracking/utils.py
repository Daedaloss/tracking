...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun  9 16:42:29 2022
...
def rescale_img(mask, frame, mask_size=256):
    ...
    rectsize = [mask[2], mask[3]] #width, height

    rectsize = np.asarray(rectsize)
    scale = mask_size / rectsize.max()

    cutout = frame[mask[0] : mask[0] + mask[2], mask[1] - mask[3] : mask[1], :]
    if not cutout.size: # empty cutout
        return None
    img_help = rescale(cutout, scale, channel_axis=-1)#was multichannel
    ...
    x_diff = mask_size - img_help.shape[0]
    y_diff = mask_size - img_help.shape[1]
    ## Ugly hack alert 
    x_fix = x_diff % 2 
    y_fix = y_diff % 2 
    x_diff = x_diff//2
    ...
    padded_img = np.pad(img_help, [(x_diff,x_diff+x_fix), (y_diff, y_diff+y_fix), (0,0)], mode='constant')
    #cv2.imshow('Unpadded Image', img_help)
    #cv2.waitKey(0)
